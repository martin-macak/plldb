...
def assume_debugger_role() -> boto3.Session:
    ...
    try:
        response = sts.assume_role(
            RoleArn=f"arn:aws:iam::{boto3.client('sts').get_caller_identity()['Account']}:role/PLLDBDebuggerRole",
            RoleSessionName="plldb-lambda-runtime",
            ExternalId="plldb-debugger"
        )
...
def send_websocket_notification(
    ...
    apigateway = session.client("apigatewaymanagementapi",
